...
pluralDF = wordsDF.withColumn('word', concat(wordsDF.word, lit("s")))
pluralDF.show()
...
# N.B. This doesn't seem to exist anymore! 
...
# TODO: Replace <FILL IN> with appropriate code
averageCount = wordCountsDF.groupBy().mean().first()
...
def removePunctuation(column):
    ...
    """
    return trim(regexp_replace(lower(column), "[^a-zA-Z0-9\s]", "")).alias("sentence")
